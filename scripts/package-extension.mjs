...
await copyFile('LICENSE', 'dist/extension/chrome-mv3/LICENSE');
...
archive.pipe(output);
const files = [
  ...(await filesIn(source)).map((path) => ({ path, name: relative(source, path).split(sep).join('/') })),
  { path: 'LICENSE', name: 'LICENSE' }
];
for (const { path, name } of files.sort((left, right) => left.name.localeCompare(right.name))) {
